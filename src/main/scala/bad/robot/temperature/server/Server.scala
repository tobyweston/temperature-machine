...
object Server extends App {
  ...
  def http(temperatures: AllTemperatures, connections: Connections)(implicit monitored: List[Host]): IO[HttpServer] = {
    val port = 11900
    for {
      server <- HttpServer(port, monitored, temperatures, connections)
      _      <- info(s"HTTP Server started on http://${InetAddress.getLocalHost.getHostAddress}:$port")
      _      <- server.awaitShutdown()
    } yield server
  }

  def server(temperatures: AllTemperatures, connections: Connections, sensors: List[SensorFile])(implicit monitored: List[Host]) = {
    for {
      ...
      _ <- graphing
      _ <- exportJson
      _ <- http(temperatures, connections)
    } yield ()
  }
  ...
  findSensorsAndExecute(server(temperatures, connections, _)(hosts)).leftMap(error => Log.error(error.message))
