...
class CurrentAverageTemperatureTest extends Specification {
  ...
  "Averages a single temperature" >> {
    ...
    val service = TemperatureEndpoint.service(stub(\/-(List(Temperature(56.34)))))
  ...
  "Averages several temperatures" >> {
    val request = Request(GET, Uri.uri("/temperature"))
    val service = TemperatureEndpoint.service(stub(\/-(List(
      Temperature(25.344),
      Temperature(23.364),
      Temperature(21.213)
    ))))
  ...
  "General Error reading temperatures" >> {
    val request = Request(GET, Uri.uri("/temperature"))
    val service = TemperatureEndpoint.service(stub(-\/(SensorError("An example error"))))
    val response = service(request).run
