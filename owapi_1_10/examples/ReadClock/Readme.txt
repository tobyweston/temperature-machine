...
------------------------------------------------
| 1-Wire Clock Demo (ReadClock) |
------------------------------------------------

I. Description
=------------=
Find and read all 1-Wire devices that implement
the ClockContainer interface.
...
ReadClock.class       - prebuilt Desktop class
\src\ReadClock.java   - source for this example
\tini\ReadClock.tini  - prebuilt TINI class (for TINI 1.02 or later)
...
1. Use the provided 'run.bat' to start this application.
   It will automatically search for a Clock on the Default 1-Wire port.
   A 1-Wire Adapter/Port can be specified on the command line
   with the following syntax.

     Clock Container Demo
       Usage: 
          java ReadClock ADAPTER_PORT
            ADAPTER_PORT is a String that contains the name of the
            adapter you would like to use and the port you would like
            to use, for example: 
                   java ReadClock {DS1410E}_LPT1
...
1. Load ReadClock from the examples\ReadClock\tini directory
...
3. To get command line systax execute ReadClock without any parameter
   'java ReadClock.tini'.  Parameters can be added as described above
   under Desktop.
...
4. Use the 'tini_build.bat' to rebuild this example for TINI.  Note that this 
   ...
   'tini_build.bat' compiles in all known types of Clocks.  The end result 
